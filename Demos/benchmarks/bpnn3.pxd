...
cdef class NN:
    ...
    @cython0.locals(i=Py_ssize_t, j=Py_ssize_t, k=Py_ssize_t)
    cpdef update(self, list inputs)

    @cython0.locals(i=Py_ssize_t, j=Py_ssize_t, k=Py_ssize_t, change=double)
    cpdef double backPropagate(self, list targets, double N, M)

    @cython0.locals(i=Py_ssize_t, p=list, error=double)
