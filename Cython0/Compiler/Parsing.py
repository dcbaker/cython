...
import cython0
cython0.declare(Nodes=object, ExprNodes=object, EncodedString=object,
               ...
               partial=object, reduce=object, _IS_PY3=cython0.bint, _IS_2BYTE_UNICODE=cython0.bint,
...
comparison_ops = cython0.declare(set, set([
    '<', '>', '==', '>=', '<=', '<>', '!=',
    'in', 'is', 'not'
]))
...
expr_terminators = cython0.declare(set, set([
    ')', ']', '}', ':', '=', 'NEWLINE']))
...
imported_name_kinds = cython0.declare(set, set(['class', 'struct', 'union']))
...
statement_terminators = cython0.declare(set, set([';', 'NEWLINE', 'EOF']))
...
inequality_relations = cython0.declare(set, set(['<', '<=', '>', '>=']))
...
calling_convention_words = cython0.declare(
...
basic_c_type_names = cython0.declare(
    set, set(["void", "char", "int", "float", "double", "bint"]))

special_basic_c_types = cython0.declare(dict, {
...
sign_and_longness_words = cython0.declare(
    set, set(["short", "long", "signed", "unsigned"]))

base_type_start_words = cython0.declare(
    set,
    basic_c_type_names
    | sign_and_longness_words
    | set(special_basic_c_types))

struct_enum_union = cython0.declare(
...
supported_overloaded_operators = cython0.declare(set, set([
    '+', '-', '*', '/', '%',
    '++', '--', '~', '|', '&', '^', '<<', '>>', ',',
    '==', '!=', '>=', '>', '<=', '<',
    '[]', '()', '!', '=',
...
c_arg_list_terminators = cython0.declare(set, set(['*', '**', '.', ')', ':']))
...
_match_compiler_directive_comment = cython0.declare(object, re.compile(
    ...
    if s.context.language_level is None:
        ...
        if pos[0].filename:
            ...
            warnings.warn(
                ...
                FutureWarning,
                stacklevel=1 if cython0.compiled else 2,
            )
