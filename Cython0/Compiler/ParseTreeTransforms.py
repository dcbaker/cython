...
import cython0
cython0.declare(PyrexTypes=object, Naming=object, ExprNodes=object, Nodes=object,
...
class InterpretCompilerDirectives(CythonTransform):
    ...
    - @cython0.directivename decorators
    - with cython0.directivename: statements
    ...
    def is_parallel_directive(self, full_name, pos):
        """
        Checks to see if fullname (e.g. cython0.parallel.prange) is a valid
        parallel directive. If it is a star import it also updates the
        parallel_directives.
        """
        result = (full_name + ".").startswith("cython0.parallel.")

        if result:
            directive = full_name.split('.')
            if full_name == u"cython0.parallel":
                self.parallel_directives[u"parallel"] = u"cython0.parallel"
            elif full_name == u"cython0.parallel.*":
                for name in self.valid_parallel_directives:
                    self.parallel_directives[name] = u"cython0.parallel.%s" % name
    ...
    def visit_CImportStatNode(self, node):
        if node.module_name == u"cython0":
            self.cython_module_names.add(node.as_name or u"cython0")
        elif node.module_name.startswith(u"cython0."):
            if node.module_name.startswith(u"cython0.parallel."):
                error(node.pos, node.module_name + " is not a module")
            if node.module_name == u"cython0.parallel":
                if node.as_name and node.as_name != u"cython0":
                    self.parallel_directives[node.as_name] = node.module_name
                else:
                    self.cython_module_names.add(u"cython0")
                    self.parallel_directives[
                                    u"cython0.parallel"] = node.module_name
                self.module_scope.use_utility_code(
                    UtilityCode.load_cached("InitThreads", "ModuleSetupCode.c"))
            elif node.as_name:
    ...
    def visit_FromCImportStatNode(self, node):
        if not node.relative_level and (
                node.module_name == u"cython0" or node.module_name.startswith(u"cython0.")):
            submodule = (node.module_name + u".")[7:]
            newimp = []

            for pos, name, as_name, kind in node.imported_names:
                full_name = submodule + name
                qualified_name = u"cython0." + full_name
    ...
    def visit_FromImportStatNode(self, node):
        if (node.module.module_name.value == u"cython0") or \
               node.module.module_name.value.startswith(u"cython0."):
    ...
    def visit_SingleAssignmentNode(self, node):
        if isinstance(node.rhs, ExprNodes.ImportNode):
            ...
            is_parallel = (module_name + u".").startswith(u"cython0.parallel.")

            if module_name != u"cython0" and not is_parallel:
                return node
        ...
        for name, value in directives.items():
            if name == 'locals':
                ...
                self.context.nonfatal_error(PostParseError(
                    node.pos,
                    "Cdef functions can only take cython0.locals(), "
            ...
            if directive is not None:
                if node.target is not None:
                    ...
                    if name in ('nogil', 'gil'):
                        # special case: in pure mode, "with nogil" spells "with cython0.nogil"
...
class ParallelRangeTransform(CythonTransform, SkipDeclarations):
    """
    Transform cython0.parallel stuff. The parallel_directives come from the
    module node, set there by InterpretCompilerDirectives.

        x = cython0.parallel.threadavailable()   -> ParallelThreadAvailableNode
        with nogil, cython0.parallel.parallel(): -> ParallelWithBlockNode
            print cython0.parallel.threadid()    -> ParallelThreadIdNode
            for i in cython0.parallel.prange(...):  -> ParallelRangeNode
                ...
    """

    # a list of names, maps 'cython0.parallel.prange' in the code to
    # ['cython0', 'parallel', 'prange']
    ...
    directive_to_node = {
        u"cython0.parallel.parallel": Nodes.ParallelWithBlockNode,
        # u"cython0.parallel.threadsavailable": ExprNodes.ParallelThreadsAvailableNode,
        u"cython0.parallel.threadid": ExprNodes.ParallelThreadIdNode,
        u"cython0.parallel.prange": Nodes.ParallelRangeNode,
    }
    ...
    def get_directive_class_node(self, node):
        ...
        E.g. for a cython0.parallel.prange() call we return ParallelRangeNode
        """
    ...
    def visit_WithStatNode(self, node):
        "Rewrite with cython0.parallel.parallel() blocks"
    ...
    def visit_ForInStatNode(self, node):
        "Rewrite 'for i in cython0.parallel.prange(...):'"
...
if VALUE is not None:
    ...
    def visit_FuncDefNode(self, node):
        ...
        analyse the directive_locals set by @cython0.locals().
...
class TransformBuiltinMethods(EnvTransform):
    """
    Replace Cython's own cython0.* builtins by the corresponding tree nodes.
    """
...
class DebugTransform(CythonTransform):
    ...
    def __init__(self, context, options, result):
        ...
        # (see Cython0.Debugger.debug_output.CythonDebugWriter)
    ...
    def visit_ModuleNode(self, node):
        ...
        # Cython0.Compiler.ModuleNode.ModuleNode._serialize_lineno_map
        return node
