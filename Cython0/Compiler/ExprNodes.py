...
import cython0
cython0.declare(error=object, warning=object, warn_once=object, InternalError=object,
...
class NameNode(AtomicExprNode):
    ...
    def declare_from_annotation(self, env, as_target=False):
        ...
        The ambiguous Python types 'int' and 'long' are ignored and the 'cython0.int' form must be used instead.
        """
...
#-------------------------------------------------------------------
#
#  Parallel nodes (cython0.parallel.thread(savailable|id))
#
#-------------------------------------------------------------------
...
class ParallelThreadsAvailableNode(AtomicExprNode):
    ...
    Implements cython0.parallel.threadsavailable(). If we are called from the
...
class ParallelThreadIdNode(AtomicExprNode): #, Nodes.ParallelNode):
    """
    Implements cython0.parallel.threadid()
    """
...
ERR_NOT_POINTER = "Can only create cython0.array from pointer or array"
ERR_BASE_TYPE = "Pointer base type does not match cython0.array base type"
...
class CythonArrayNode(ExprNode):
    ...
    creates a fortran-contiguous cython0.array.
    ...
    def generate_result_code(self, code):
        ...
        if self.operand.type.is_ptr:
            ...
            code.putln(    'PyErr_SetString(PyExc_ValueError,'
                                '"Cannot create cython0.array from NULL pointer");')
