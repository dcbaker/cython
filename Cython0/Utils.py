...
def get_cython_cache_dir():
    ...
    if parent and os.path.isdir(parent):
        return os.path.join(parent, 'cython0')

    # last fallback: ~/.cython
    return os.path.expanduser(os.path.join('~', '.cython0'))
...
def raise_error_if_module_name_forbidden(full_module_name):
    #it is bad idea to call the pyx-file cython0.pyx, so fail early
    if full_module_name == 'cython0' or full_module_name.startswith('cython0.'):
