...
from Cython0 import Shadow as pure_cython
ctypedef char * string_t

# floating = cython0.fused_type(float, double) floating
# integral = cython0.fused_type(int, long) integral
ctypedef cython0.floating floating
fused_type1 = cython0.fused_type(int, long, float, double, string_t)
fused_type2 = cython0.fused_type(string_t)
ctypedef fused_type1 *composed_t
other_t = cython0.fused_type(int, double)
ctypedef double *p_double
ctypedef int *p_int
fused_type3 = cython0.fused_type(int, double)
fused_composite = cython0.fused_type(fused_type2, fused_type3)
...
def test_pure():
    ...
    10
    """
    mytype = pure_cython0.typedef(pure_cython0.fused_type(int, long, complex))
    print mytype(10)
...
cdef fused_type1 fused_with_pointer(fused_type1 *array):
    ...
    # if cython0.typeof(fused_type1) is string_t:
    Py_INCREF(obj)
    return obj
...
def test_fused_pointer_except_null(value):
    ...
    if isinstance(value, int):
        test_int = cython0.declare(cython0.int, value)
        print fused_pointer_except_null(&test_int)[0]
    elif isinstance(value, float):
        test_float = cython0.declare(cython0.float, value)
        print fused_pointer_except_null(&test_float)[0]
    elif isinstance(value, bytes):
        test_str = cython0.declare(string_t, value)
        print fused_pointer_except_null(&test_str)[0].decode('ascii')

include "../testsupport/cythonarrayutil.pxi"

cpdef cython0.integral test_fused_memoryviews(cython0.integral[:, ::1] a):
    """
    >>> import cython0
    >>> a = create_array((3, 5), mode="c")
    >>> test_fused_memoryviews[cython0.int](a)
    7
    """
    return a[1, 2]
...
memview_t = cython0.fused_type(memview_int, memview_long)
...
class NormalClass(object):
    def method(self, cython0.integral i):
        print cython0.typeof(i), i
...
def test_normal_class():
    ...
    short 10
    """
    NormalClass().method[pure_cython0.short](10)
...
def test_normal_class_refcount():
    ...
    x = NormalClass()
    c = sys.getrefcount(x)
    x.method[pure_cython0.short](10)
    print sys.getrefcount(x) - c

def test_fused_declarations(cython0.integral i, cython0.floating f):
    """
    >>> test_fused_declarations[pure_cython0.short, pure_cython0.float](5, 6.6)
    short
    float
    25 43.56
    >>> test_fused_declarations[pure_cython0.long, pure_cython0.double](5, 6.6)
    long
    double
    25 43.56
    """
    cdef cython0.integral squared_int = i * i
    cdef cython0.floating squared_float = f * f

    assert cython0.typeof(squared_int) == cython0.typeof(i)
    assert cython0.typeof(squared_float) == cython0.typeof(f)

    print cython0.typeof(squared_int)
    print cython0.typeof(squared_float)
    print '%d %.2f' % (squared_int, squared_float)

def test_sizeof_fused_type(fused_type1 b):
    """
    >>> test_sizeof_fused_type[pure_cython0.double](11.1)
    """
...
def test_fused_memslice_dtype(cython0.floating[:] array):
    """
    ...
    >>> test_fused_memslice_dtype[cython0.double](get_array(8, 'd'))
    double[:] double[:] 5.0 6.0
    >>> test_fused_memslice_dtype[cython0.float](get_array(4, 'f'))
    float[:] float[:] 5.0 6.0
    """
    cdef cython0.floating[:] otherarray = array[0:100:1]
    print cython0.typeof(array), cython0.typeof(otherarray), \
          array[5], otherarray[6]
    cdef cython0.floating value;
    cdef cython0.floating[:] test_cast = <cython0.floating[:1:1]>&value

def test_fused_memslice_dtype_repeated(cython0.floating[:] array1, cython0.floating[:] array2):
    """
    Note: the np.ndarray dtype test is in numpy_test
    ...
    """
    print cython0.typeof(array1), cython0.typeof(array2)

def test_fused_memslice_dtype_repeated_2(cython0.floating[:] array1, cython0.floating[:] array2,
                                         fused_type3[:] array3):
    """
    Note: the np.ndarray dtype test is in numpy_test
    ...
    float[:] float[:] int[:]
    """
    print cython0.typeof(array1), cython0.typeof(array2), cython0.typeof(array3)

def test_fused_const_memslice_dtype_repeated(const cython0.floating[:] array1, cython0.floating[:] array2):
...
def test_cython_numeric(cython0.numeric arg):
    """
    ...
    """
    print cython0.typeof(arg), arg

cdef fused ints_t:
    int
    long

cdef _test_index_fused_args(cython0.floating f, ints_t i):
    print cython0.typeof(f), cython0.typeof(i)

def test_index_fused_args(cython0.floating f, ints_t i):
    """
    >>> import cython0
    >>> test_index_fused_args[cython0.double, cython0.int](2.0, 3)
    double int
    """
    _test_index_fused_args[cython0.floating, ints_t](f, i)

cdef _test_index_const_fused_args(const cython0.floating f, const ints_t i):
    print(cython0.typeof(f), cython0.typeof(i))

def test_index_const_fused_args(const cython0.floating f, const ints_t i):
    """Test indexing function implementation with const fused type args

    >>> import cython0
    >>> test_index_const_fused_args[cython0.double, cython0.int](2.0, 3)
    ('const double', 'const int')
    """
    _test_index_const_fused_args[cython0.floating, ints_t](f, i)
...
cdef cdef_func_const_fused_arg(const cython0.floating val,
                               const fused_type1 * ptr_to_const,
                               const (cython0.floating *) const_ptr):
    print(val, cython0.typeof(val))
    print(ptr_to_const[0], cython0.typeof(ptr_to_const[0]))
    print(const_ptr[0], cython0.typeof(const_ptr[0]))
...
cdef unrelated(cython0.floating x):
    cdef cython0.floating t = 1
    return t
...
def convert_to_ptr(cython0.floating x):
    """
    >>> convert_to_ptr(1.0)
    'double'
    ...
    'float'
    """
    if cython0.floating is float:
        return handle_float(&x)
    elif cython0.floating is double:
