...
def str_join(str s, args):
    ...
    """
    result = s.join(args)
    assert cython0.typeof(result) == 'basestring object', cython0.typeof(result)
    return result
...
def mod_format(str s, values):
    ...
    123
    """
    assert cython0.typeof(s % values) == 'basestring object', cython0.typeof(s % values)
    return s % values
...
def mod_format_literal(values):
    ...
    True
    """
    assert cython0.typeof('abc%sdef' % values) == 'basestring object', cython0.typeof('abc%sdef' % values)
