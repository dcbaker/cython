...
C library functions from Cython0 code.  For a longer and more
...
the additional advantage that Cython0 already defines many such
...
Cython0's source package
`Cython0/Includes/ <https://github.com/cython/cython/tree/master/Cython0/Includes>`_.
They are stored in ``.pxd`` files, the standard way to provide reusable
Cython0 declarations that can be shared across modules
(see :ref:`sharing-declarations`).

Cython0 also has a complete set of declarations for CPython's C-API.
...
Cython0 also provides declarations for the C math library:
...
If you want to access C code for which Cython0 does not provide a ready
...
to Cython0 code and instructs Cython0 to generate C code that includes
the ``math.h`` header file.  The C compiler will see the original
declaration in ``math.h`` at compile time, but Cython0 does not parse
...
Cython0 generates is properly linked against the shared or static
library.

Note that you can easily export an external C function from your Cython0
module by declaring it as ``cpdef``.  This generates a Python wrapper
for it and adds it to the module dict.  Here is a Cython0 module that
...
file that belongs to the Cython0 module (i.e. that has the same name,
see :ref:`sharing-declarations`).
This allows the C declaration to be reused in other Cython0 modules,
...
Both C and Cython0 support signature declarations without parameter
names like this::

  cdef extern from "string.h":
      char* strstr(const char*, const char*)

However, this prevents Cython0 code from calling it with keyword
