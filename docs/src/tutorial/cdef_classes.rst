...
To support object-oriented programming, Cython0 supports writing normal
Python classes exactly as in Python:

.. literalinclude:: ../../examples/tutorial/cdef_classes/math_function.py

Based on what Python calls a "built-in type", however, Cython0 supports
...
way around.  Cython0 requires to know the complete inheritance
...
Cython0 code and pure Python code.
...
fast for use from Cython0 and one slower for use from Python. Then:
...
Some notes on our new implementation of ``evaluate``:
 ...
 - The fast method dispatch here only works because ``evaluate`` was
   ...
   ``SinOfSquareFunction``, the code would still work, but Cython0
 ...
 - Since the argument is typed, we need to check whether it is
   ...
   when the ``evaluate`` method was looked up, but Cython0 would instead
...
Attributes in cdef classes behave differently from attributes in regular classes:
 ...
 - Attributes are by default only accessible from Cython0 (typed access)
