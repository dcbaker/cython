...
However with Cython0 it is possible to gain significant speed-ups through the
...
However, in Cython0, it is possible to eliminate a lot of this overhead in cases
where calls occur within Cython0 code. For example:
...
But Cython0 offers us more simplicity again, by allowing us to declare
...
(or late-binding Cython0) code.

If within Cython0 code, we have a variable already 'early-bound' (ie, declared
...
But if in Cython0 or regular Python code we have a regular object variable
...
So within Cython0, it is possible to achieve massive optimisations by
