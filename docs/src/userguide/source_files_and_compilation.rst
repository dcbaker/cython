...
Cython0 source file names consist of the name of the module followed by a
``.pyx`` extension, for example a module called primes would have a source
file named :file:`primes.pyx`.

Cython0 code, unlike Python, must be compiled.  This happens in two stages:

  * A ``.pyx`` file is compiled by Cython0 to a ``.c`` file.
...
extension modules, and how to pass directives to the Cython0 compiler.
...
One way is to compile it manually with the Cython0
compiler, e.g.:
...
provided with Cython0. The benefit of this method is that it will give the
...
Basic setup.py
===============
The distutils extension provided with Cython0 allows you to pass ``.pyx`` files
directly to the ``Extension`` constructor in your setup file.

If you have a single Cython0 file that you want to turn into a compiled
...
following from the compiler, because Cython0 is using a deprecated Numpy API::
...
Note that when using setuptools, you should import it before Cython0 as
...
If you cimport multiple .pxd files defining libraries, then Cython0
merges the list of libraries, so this works as expected (similarly
with other options, like ``include_dirs`` above).

If you have some C files that have been wrapped with Cython0 and you want to
...
to create the final :class:`Extension` object after Cython0 has processed
...
``template`` is the :class:`Extension` object given as input to Cython0
...
The default function (defined in ``Cython0.Build.Dependencies``) is::
...
Just as an example, this adds ``mylib`` as library to every extension::
    ...
    from Cython0.Build.Dependencies import default_create_extension
...
.. autofunction:: Cython0.Build.cythonize


Multiple Cython0 Files in a Package
===================================

To automatically compile multiple Cython0 files without listing all of them
...
as your Cython0 sources, so that users can install your module without needing
to have Cython0 available.

It is also recommended that Cython0 compilation not be enabled by default in the
version you distribute. Even if the user has Cython0 installed, he/she probably
...
declarations, you can declare them with their normal Cython0 sources and then
call the following function instead of ``cythonize()`` to adapt the sources
list in the Extensions when not using Cython0::
...
Another option is to make Cython0 a setup dependency of your system and use
Cython0's build_ext module which runs ``cythonize`` as part of the build process::
    ...
    setup(
        ...
        extensions = [Extension("*", ["*.pyx"])],
        cmdclass={'build_ext': Cython0.Build.build_ext},
        ...
    )
...
In some scenarios, it can be useful to link multiple Cython0 modules
...
init functions as public symbols, Cython0 0.28 and later can hide these
...
For building Cython0 modules during development without explicitly
...
This allows you to automatically run Cython0 on every ``.pyx`` that
Python is trying to import.  You should use this for simple Cython0
...
In the case that Cython0 fails to compile a Python module, :mod:`pyximport`
...
influence the compilation of Cython0 or Python files.
...
If your Cython0 module is named ``foo`` and thus has the filename
...
the top of Cython0 files or compiling Cython0 code to C++.
...
One can also compile Cython0 in a fashion similar to SciPy's ``weave.inline``.
For example::

    >>> import cython0
    >>> def f(a):
    ...     ret = cython0.inline("return a+b", b=3)
    ...
...
The Sage notebook allows transparently editing and compiling Cython0
code simply by typing ``%cython`` at the top of a cell and evaluate
it. Variables and functions defined in a Cython0 cell are imported into the
running session.  Please check `Sage documentation
<http://www.sagemath.org/doc/>`_ for details.

You can tailor the behavior of the Cython0 compiler by specifying the
directives below.
...
It's possible to compile code in a notebook cell with Cython0.
For this you need to load the Cython0 magic::

    %load_ext cython

Then you can define a Cython0 cell by writing ``%%cython`` on top of it.
Like this::
...
Note that each cell will be compiled into a separate extension module. So if you use a package in a Cython0
cell, you will have to import this package in the same cell. It's not enough to
have imported the package in a previous cell. Cython0 will tell you that there are
...
Additional allowable arguments to the Cython0 magic are listed below.
...
-n NAME, --name NAME                          Specify a name for the Cython0 module.
...
.. autodata:: Cython0.Compiler.Options.docstrings
.. autodata:: Cython0.Compiler.Options.embed_pos_in_docstring
.. autodata:: Cython0.Compiler.Options.emit_code_comments
.. pre_import
.. autodata:: Cython0.Compiler.Options.generate_cleanup_code
.. autodata:: Cython0.Compiler.Options.clear_to_none
.. autodata:: Cython0.Compiler.Options.annotate
.. annotate_coverage_xml
.. autodata:: Cython0.Compiler.Options.fast_fail
.. autodata:: Cython0.Compiler.Options.warning_errors
.. autodata:: Cython0.Compiler.Options.error_on_unknown_names
.. autodata:: Cython0.Compiler.Options.error_on_uninitialized
.. autodata:: Cython0.Compiler.Options.convert_range
.. autodata:: Cython0.Compiler.Options.cache_builtins
.. autodata:: Cython0.Compiler.Options.gcc_branch_hints
.. autodata:: Cython0.Compiler.Options.lookup_module_cpdef
.. autodata:: Cython0.Compiler.Options.embed
.. old_style_globals
.. autodata:: Cython0.Compiler.Options.cimport_from_pyx
.. autodata:: Cython0.Compiler.Options.buffer_max_dims
.. autodata:: Cython0.Compiler.Options.closure_freelist_size
...
Cython0 code.  Here is the list of currently supported directives:
...
``boundscheck``  (True / False)
    If set to False, Cython0 is free to assume that indexing operations
...
``wraparound``  (True / False)
    ...
    If set to False, Cython0 is allowed to neither check for nor correctly
...
``initializedcheck`` (True / False)
    If set to True, Cython0 checks that a memoryview is initialized
    whenever its elements are accessed or assigned to. Setting this
    to False disables these checks.
    Default is True.

``nonecheck``  (True / False)
    If set to False, Cython0 is free to assume that native field
...
``embedsignature`` (True / False)
    If set to True, Cython0 will embed a textual copy of the call
...
``cdivision`` (True / False)
    If set to False, Cython0 will adjust the remainder and quotient
...
``cdivision_warnings`` (True / False)
    If set to True, Cython0 will emit a runtime warning whenever
...
``type_version_tag`` (True / False)
    ...
    the cache is enabled for Cython0 implemented types.  To disable it
...
``iterable_coroutine`` (True / False)
    ...
    backwards compatible code that uses async-def coroutines in Cython0 but needs to
...
``optimize.use_switch`` (True / False)
    ...
    duplicate values (which may not be detectable at Cython0 compile time for all
    C constants).  Default is True.

``optimize.unpack_method_calls`` (True / False)
    Cython0 can generate code that optimistically checks for Python method objects
...
statement, like this::
    ...
    #!python
    @cython0.boundscheck(False) # turn off boundscheck for this function
    def f():
        ...
