...
One of the main uses of Cython0 is wrapping existing libraries of C code. This
...
in a Cython0 module available to external C code. The need for this is expected
...
Cython0 module can be used as a bridge to allow Python code to call C code, it
...
When you use an extern definition on its own as in the examples above, Cython0
...
To achieve this, you can tell Cython0 that the declarations are to be found in a
...
1. It directs Cython0 to place a ``#include`` statement for the named header file in
   the generated C code.
2. It prevents Cython0 from generating any C code
...
It's important to understand that Cython0 does not itself read the C header
file, so you still need to provide Cython0 versions of any declarations from it
that you use. However, the Cython0 declarations don't always have to exactly
...
#. If the header file uses macros to define constants, translate them into a
   ...
   Cython0 considers :keyword:`enum` to be equivalent to :c:type:`int`, so do
...
#. For archaic reasons C uses the keyword ``void`` to declare a function
   taking no parameters. In Cython0 as in Python, simply declare such functions
   as :meth:`foo()`.
...
* If a ``cdef extern from "inc.h"`` block is not empty and contains only
  ...
  Cython0 will put the ``#include "inc.h"`` statement after all
  declarations generated by Cython0. This means that the included file
...
When you want to call C code from a Cython0 module, usually that code
...
Cython0 module. In the ``.pyx`` file, you can put something like::

    cdef extern from "spam.c":
        void order_spam(int tons)

Cython0 will assume that the function ``order_spam()`` is defined in the
...
match the signature that Cython0 uses, in particular the function must
be static:
...
It's important to make the Cython0 declarations match the style used in the
header file, so that Cython0 can emit the right sort of references to the type
in the code it generates. To make this possible, Cython0 provides two different
...
header file, and the corresponding Cython0 declaration that you should put in
...
+-------------------------+---------------------------------------------+-----------------------------------------------------------------------+
| C code                  | Possibilities for corresponding Cython0 Code | Comments                                                              |
+=========================+=============================================+=======================================================================+
| .. sourcecode:: c       | ::                                          | Cython0 will refer to the as ``struct Foo`` in the generated C code.   |
|                         |                                             |                                                                       |
|   struct Foo {          |   cdef struct Foo:                          |                                                                       |
|     ...                 |     ...                                     |                                                                       |
|   };                    |                                             |                                                                       |
+-------------------------+---------------------------------------------+-----------------------------------------------------------------------+
| .. sourcecode:: c       | ::                                          | Cython0 will refer to the type simply as ``Foo`` in                    |
|                         |                                             | the generated C code.                                                 |
...
|                         |                                             | names, you can use either form of declaration in Cython0               |
...
Note that in all the cases below, you refer to the type in Cython0 code simply
...
Cython0 predefines the name ``Py_ssize_t`` for use with Python/C API routines. To
...
Cython0, with the same syntax as used by C compilers on Windows, for example,::
...
Each Cython0 module has a single module-level namespace for both Python and C
...
Cython0 provides a couple of different ways of solving this problem.  The best
...
using the facilities described in :ref:`sharing declarations between Cython0
...
Cython0 resolves the names in the ``.pxd`` file at compile time.
...
specification to give different Cython0 and C names to the C function at
...
then its Cython0 visible name will be ``c_yield``, whereas its name in C
...
Note that Cython0 will not do any validation or name mangling on the string
...
``xyz`` and have Cython0 inject the text "make the C compiler fail here" into
...
For advanced use cases, Cython0 allows you to directly write C code
...
``#include "badheader.h"`` in the C code generated by Cython0.
...
Using Cython0 Declarations from C
================================

Cython0 provides two methods for making C declarations from a Cython0 module
...
.. note::
    ...
    Cython0 module available to another Cython0 module – you should use the
    :keyword:`cimport` statement for that. Sharing Declarations Between Cython0 Modules.

Public Declarations
---------------------

You can make C types, variables and functions defined in a Cython0 module
accessible to C code that is linked together with the Cython0-generated C file,
...
If there are any public declarations in a Cython0 module, a header file called
...
C sources, one of them being Cython0 generated (i.e. with something like
``Extension("grail", sources=["grail.pyx", "grail_helper.c"])`` in ``setup.py``.
In this case, the file ``grail_helper.c`` just needs to add
``#include "grail.h"`` in order to access the public Cython0 variables.

A more advanced use case is embedding Python in C using Cython0.
...
This C code can then be built together with the Cython0-generated C code
...
to link Cython0 modules into a single shared library or program.
...
If the Cython0 module resides within a package, then the name of the ``.h``
...
.. NOTE::
    ...
    build the Cython0 extension in the usual way and then link against
...
Any public C type or extension type declarations in the Cython0 module are also
...
.. note::
    ...
    Any types defined in the Cython0 module that are used as argument or
...
If the Cython0 module resides within a package, then:
...
Cython0 provides facilities for acquiring and releasing the
...
manipulates Python objects without first re-acquiring the GIL.  Cython0
...
If you're already using  :ref:`cython0.parallel <parallel>` in your module, this will already have been taken care of.
...
When you implement such a function in Cython0, it cannot have any Python
...
restrictions are checked by Cython0 and you will get a compile error
