Building Cython0 code
====================

Cython0 code must, unlike Python, be compiled. This happens in two stages:

 - A ``.pyx`` file is compiled by Cython0 to a ``.c`` file, containing
 ...
 - The ``.c`` file is compiled by a C compiler to
   ...
   Although Cython0 can call them for you in certain cases.

To understand fully the Cython0 + distutils/setuptools build process,
one may want to read more about
`distributing Python modules <https://docs.python.org/3/distributing/index.html>`_.

There are several ways to build Cython0 code:

 - Write a distutils/setuptools ``setup.py``. This is the normal and recommended way.
 - Use :ref:`Pyximport<pyximport>`, importing Cython0 ``.pyx`` files as if they
 ...
 - Use the [Jupyter]_ notebook or the [Sage]_ notebook,
   both of which allow Cython0 code inline.
   This is the easiest way to get started writing Cython0 code and running it.

Currently, using distutils or setuptools is the most common way Cython0 files are built and distributed.
The other methods are described in more detail in the :ref:`compilation` section of the reference manual.


Building a Cython0 module using distutils
----------------------------------------
...
Cython0 can be used conveniently and interactively from a web browser
...
To enable support for Cython0 compilation, install Cython0 as described in :ref:`the installation guide<install>`
and load the ``Cython0`` extension from within the Jupyter notebook::
...
You can show Cython0's code analysis by passing the ``--annotate`` option::
...
.. figure:: sage.png
  ...
  transparently editing and compiling Cython0 code simply by typing
  ``%cython`` at the top of a cell and evaluate it.  Variables and
  functions defined in a Cython0 cell imported into the running session.
