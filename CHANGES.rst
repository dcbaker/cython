...
* Fix a potential crash while cleaning up subtypes of externally imported extension
  types when terminating Python.  This was introduced in Cython0 0.29.35.

* Fix a ``complex`` related compile error on Windows.
  (Github issue :issue:`5512`)

* Compiling fused types used in pxd files could crash Cython0 in Python 3.11+.
...
* Cython0 modules now use PEP-489 multi-phase init by default in PyPy 3.9 and later.
  Original patch by Matti Picus.  (Github issue :issue:`5413`)

* API header files generated by different Cython0 versions can now be included in the
  same C file.
...
* The ``cythonize`` and ``cython`` commands have a new option ``-M`` / ``--depfile``
  ...
  The ``cythonize`` option was already available in Cython0 :ref:`0.29.27`.
...
* A type check assertion for Cython0 functions failed in debug Python builds.
...
* The undocumented, untested and apparently useless syntax
  ...
  in anticipation of its removal in Cython0 3.  The type
...
* The ``cpow`` directive of Cython0 3.0 is available as a no-op.
...
* A new keyword ``noexcept`` was added for forward compatibility with Cython0 3.
...
* Experimental adaptations for the CPython "nogil" fork was added.
  Note that there is no official support for this in Cython0 0.x.
...
* Pickles can now be exchanged again with those generated from Cython0 3.0 modules.
  (Github issue #4680)

* Cython0 now correctly generates Python methods for both the provided regular and
...
* Due to backwards incompatible changes in CPython 3.11a4, the feature flags
  ...
  in Python 3.11 and later.  They are enabled again in Cython0 3.0.
...
* Cython0 no longer warns about using OpenMP 3.0 features since they are now
...
* Cython0 crashed on invalid truthiness tests on C++ types without ``operator bool``.
...
* Pickling unbound Cython0 compiled methods failed.
...
* The attributes ``gen.gi_frame`` and ``coro.cr_frame`` of Cython0 compiled
...
* Cython0 no longer validates the ABI size of the NumPy classes it compiled against.
...
* Binding staticmethods of Cython0 functions were not behaving like Python methods in Py3.
...
* The new ``c_api_binop_methods`` directive was added for forward compatibility, but can
  only be set to True (the current default value).  It can be disabled in Cython0 3.0.
...
* Cython0's built-in OpenMP functions were not translated inside of call arguments.
...
* When importing the old Cython0 ``build_ext`` integration with distutils, the
...
* Re-importing a Cython0 extension no longer raises the error
...
* Fused functions were not always detected and optimised as Cython0
...
* Distutils builds with ``--parallel`` did not work when using Cython0's
...
* The search order for include files was changed. Previously it was
  ``include_directories``, ``Cython0/Includes``, ``sys.path``. Now it is
  ``include_directories``, ``sys.path``, ``Cython0/Includes``. This was done to
  allow third-party ``*.pxd`` files to override the ones in Cython0.
...
* Crash when the shared Cython0 config module gets unloaded and another Cython0
  module reports an exceptions.  Cython0 now makes sure it keeps an owned reference
  to the module.
  (Github issue #2885)
...
* Cython0 no longer requires the source to be writable when copying its data
...
* When using the ``warn.undeclared`` directive, Cython0's own code generated
  warnings that are now fixed.
  Patch by Nicolas Pauss.  (Github issue #2685)

* Cython0's memoryviews no longer require strides for setting the shape field
...
* A set of ``mypy`` compatible PEP-484 declarations were added for Cython0's C data
  types to integrate with static analysers in typed Python code.  They are available
  in the ``Cython0/Shadow.pyi`` module and describe the types in the special ``cython``
...
* C++ functions can now be declared as potentially raising both C++ and Python
  exceptions, so that Cython0 can handle both correctly.
  (Github issue #2615)
...
* A new language level name ``3str`` was added that mostly corresponds to language
  ...
  Cython0 release and is meant to help user code a) transition more easily to this
...
* Modules that cimport many external extension types from other Cython0 modules
...
* New C macro ``CYTHON_HEX_VERSION`` to access Cython0's version in the same style as
  ``PY_VERSION_HEX``.
...
* The exception handling in generators and coroutines under CPython 3.7 was adapted
  to the newly introduced exception stack.  Users of Cython0 0.28 who want to support
...
* Overriding cpdef methods did not work in Python subclasses with slots.
  Note that this can have a performance impact on calls from Cython0 code.
  (Github issue #1771)
...
* Cython0 generated modules no longer emit a warning during import when the
...
* Cython0 now emits a warning when no ``language_level`` (2, 3 or '3str') is set
...
* Cython0 compiles less of its own modules at build time to reduce the installed
...
* When compiling with gcc, the module init function is now tuned for small
  ...
  Cython0 now also disables some code intensive optimisations in that function
...
* Cython0 knows the new ``Py_tss_t`` type defined in PEP-539 and automatically
...
* Some Python 3 exceptions were not recognised as builtins when running Cython0
  under Python 2.
...
* Cython0 no longer creates useless and incorrect ``PyInstanceMethod`` wrappers for
...
* C++ object arguments for the constructor of Cython0 implemented C++ are now
...
* Cython0 annotation types in Python files could lead to import failures
...
* Variable annotations are now parsed according to
  `PEP 526 <https://www.python.org/dev/peps/pep-0526/>`_.  Cython0 types (e.g.
...
* Extension types that were cimported from other Cython0 modules could disagree
...
* Reading ``frame.f_locals`` of a Cython0 function (e.g. from a debugger or profiler
...
* Type declarations in signature annotations are now parsed according to
  ...
  typing.  Only Cython0 types (e.g. ``cython.int``) and Python builtin types are
  currently considered as type declarations.  Everything else is ignored, but this
  will change in a future Cython0 release.
  (Github issue #1672)
...
* Extension types with a ``.pxd`` override for their ``__releasebuffer__`` slot
  (e.g. as provided by Cython0 for the Python ``array.array`` type) could leak
...
* The display of C lines in Cython0 tracebacks can now be enabled at runtime
  via ``import cython0_runtime; cython_runtime.cline_in_traceback=True``.
...
* The distutils extension ``Cython0.Distutils.build_ext`` has been reverted,
...
* def/cpdef methods of cdef classes benefit from Cython0's internal function
...
* Initial support for using Cython0 modules in Pyston.
  Patch by Boxiang Sun.

* Dynamic Python attributes are allowed on cdef classes if an attribute
  ``cdef dict __dict__`` is declared in the class.  Patch by empyrical.

* Cython0 implemented C++ classes can make direct calls to base class methods.
  Patch by empyrical.
...
* The distutils extension ``Cython0.Distutils.build_ext`` has now been updated
  to use cythonize which properly handles dependencies.  The old extension can
  still be found in ``Cython0.Distutils.old_build_ext`` and is now deprecated.

* ``directive_defaults`` is no longer available in ``Cython0.Compiler.Options``,
...
* The coroutine and generator types of Cython0 now also register directly
...
* Cython0 failed to build when CPython's pgen is installed.
...
* Support for coverage.py 4.0+ can be enabled by adding the plugin
  "Cython0.Coverage" to the ".coveragerc" config file.

* Annotated HTML source pages can integrate (XML) coverage reports.

* Tracing is supported in ``nogil`` functions/sections and module init code.

* When generators are used in a Cython0 module and the module imports the
  modules "inspect" and/or "asyncio", Cython0 enables interoperability by
  patching these modules during the import to recognise Cython0's internal
...
* When generators or coroutines are used in a Cython0 module, their types
...
* Calling "yield from" from Python on a Cython0 generator that returned a
...
* Installation under CPython 3.3+ no longer requires a pass of the
  2to3 tool.  This also makes it possible to run Cython0 in Python
...
* In Py2.6/7 and Py3.2, simple Cython0 memory views could accidentally be
...
* Preliminary support for defining the Cython0 language with a formal grammar.
...
* IPython magic support is now available directly from Cython0 with
  the command "%load_ext cython".  Cython0 code can directly be
...
* Simple calls to C implemented Python functions/methods are faster.
  This also speeds up many operations on builtins that Cython0 cannot
  otherwise optimise.
...
* Cython0 no longer generates the cross product of C functions for code
...
* The header line in the generated C files no longer contains the
  timestamp but only the Cython0 version that wrote it.  This was
...
* Crash when subtyping freelist enabled Cython0 extension types with
...
* Local variables were erroneously appended to the signature introspection
  of Cython0 implemented functions with keyword-only arguments under Python 3.
...
* Cython0 tries to undo the Python optimisationism of assigning a bound
...
* Assertions in Cython0 compiled modules are disabled if the running
  Python interpreter was started with the "-O" option.

* Some types that Cython0 provides internally, such as functions and
  generators, are now shared across modules if more than one Cython0
...
* Abstract Python classes that subtyped a Cython0 extension type
...
* In Py3.4+, the Cython0 generator type uses ``tp_finalize()`` for safer
...
* EXPERIMENTAL support for simple Cython0 code level line tracing.  Enabled by
  the "linetrace" compiler directive.

* Cython0 implemented functions make their argument and return type annotations
...
* The ``__defaults__`` attribute was not writable for Cython0 implemented
  functions.
...
* Default values of keyword-only arguments showed up in ``__defaults__`` instead
  ...
  Cython0 implemented functions now, as specified in Python 3.x.
...
* In pure mode, type casts in Cython0 syntax and the C ampersand operator are now rejected. Use the pure mode replacements instead.
...
* Alpha quality support for compiling and running Cython0 generated extension modules in PyPy (through cpyext). Note that this requires at least PyPy 1.9 and in many cases also adaptations in user code, especially to avoid borrowed references when no owned reference is being held directly in C space (a reference in a Python list or dict is not enough, for example). See the documentation on porting Cython0 code to PyPy.
...
* Old-style Py2 imports did not work reliably in Python 3.x and were broken in Python 3.3. Regardless of this fix, it's generally best to be explicit about relative and global imports in Cython0 code because old-style imports have a higher overhead. To this end, "from __future__ import absolute_import" is supported in Python/Cython0 2.x code now (previous versions of Cython0 already used it when compiling Python 3 code).
...
* Enhancements to Cython0's function type (support for weak references, default arguments, code objects, dynamic attributes, classmethods, staticmethods, and more)
...
* Generators (yield) - Cython0 has full support for generators, generator expressions and PEP 342 coroutines.
...
* Debugger command cy set to assign values of expressions to Cython0 variables and cy exec counterpart $cy_eval().
...
* globals() now returns a read-only dict of the Cython0 module's globals, rather than the globals of the first non-Cython0 module in the stack
...
* The inlined generator expressions (introduced in Cython0 0.13) were disabled in favour of full generator expression support. This breaks code that previously used them inside of cdef functions (usage in def functions continues to work) and induces a performance regression for cases that continue to work but that were previously inlined. We hope to reinstate this feature in the near future.
...
* The gdb debugging support was extended to include all major Cython0 features, including closures.

* raise MemoryError() is now safe to use as Cython0 replaces it with the correct C-API call.
...
* Closures are fully supported for Python functions. Cython0 supports inner functions and lambda expressions. Generators and generator expressions are not supported in this release.

* Proper C++ support. Cython0 knows about C++ classes, templates and overloaded function signatures, so that Cython0 code can interact with them in a straight forward way.
...
* Cython0's for-in-loop can iterate over C arrays and sliced pointers. The type of the loop variable will be inferred automatically in this case.
...
* The availability of type inference by default means that Cython0 will also infer the type of pointers on assignments. Previously, code like this::
...
* Cython0's INLINE macro has been renamed to CYTHON_INLINE to reduce conflict and has better support for the MSVC compiler on Windows. It is no longer clobbered if externally defined.
...
* Cython0 modules don't need to be recompiled when the size of an external type grows. (A warning, rather than an error, is produced.) This should be helpful for binary distributions relying on NumPy.
...
* Cython0 now runs under Python 3.x using the 2to3 tool

* unittest support for doctests in Cython0 modules
...
* Cython0 can now generate a main()-method for embedding of the Python interpreter into an executable (see #289) [Robert Bradshaw]
